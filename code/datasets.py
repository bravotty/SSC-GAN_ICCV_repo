...
class Dataset(data.Dataset):
    def __init__(self, data_dir, base_size=64, transform=None):
        ...
        self.filenames, self.bbox, self.labels_normal = self.load_bbox()
    ...
    def load_bbox(self):
        ...
        img_label_path = os.path.join(data_dir, 'image_class_labels.txt')

        df_image_labels = pd.read_csv(img_label_path,
                                      delim_whitespace=True, header=None)
        label_from_file = df_image_labels[1].tolist()
        ...
        # same as self.filenames(self) func
        filenames_out = [fname[:-4] for fname in filenames]
        print('Load filenames from: %s (%d)' % (filepath, len(filenames_out)))

        print('Total filenames: ', len(filenames), filenames[0])
        filename_bbox = {img_file[:-4]: [] for img_file in filenames}
        filename_label_normal  =  {img_file[:-4]: [] for img_file in filenames}

        numImgs = len(filenames)
        for i in range(0, numImgs):
            label = label_from_file[i]

            bbox = df_bounding_boxes.iloc[i][1:].tolist()
            key = filenames[i][:-4]
            filename_bbox[key] = bbox

            filename_label_normal[key] = label - 1

        return filenames_out, filename_bbox, filename_label_normal
    ...
    def prepair_training_pairs(self, index):
        ...
        if self.bbox is not None:
            bbox = self.bbox[key]
            y_label = self.labels_normal[key]
        else:
            bbox = None
            y_label = None
        data_dir = self.data_dir
        ...
        rand_class = random.sample(range(50), 1)# Randomly generating child code during training
        c_code = torch.zeros([50,])
        c_code[rand_class] = 1

        rand_class = random.sample(range(10), 1)# Randomly generating pca code during training
        svd_code = torch.zeros([10,])
        svd_code[rand_class] = 1

        return fimgs, cimgs, c_code, rand_class, svd_code, y_label
